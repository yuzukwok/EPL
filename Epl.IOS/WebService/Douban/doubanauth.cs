...
namespace EnjoyPubLib.Service.Douban
{
	...
	public static partial class DoubanAPI
	{
		/// <summary>
		...
		public static string AuthGetCode(string name, string pwd, string scope = null)
		{
			...
			Utilities.AddParam(ref ub, "client_id", Contans.DoubanApiKey);
			Utilities.AddParam(ref ub, "redirect_uri", Contans.DoubanRedirectUri);
		...
		public static AuthAccessToken AuthGetToken()
		{
			...
			Utilities.AddParam(ref ub, "client_id", Contans.DoubanApiKey);
			Utilities.AddParam(ref ub, "client_secret", Contans.DoubanSecret);
			Utilities.AddParam(ref ub, "redirect_uri", Contans.DoubanRedirectUri);
		...
		/// </summary>
		/// <returns>AccessToken</returns>
		public static AuthAccessToken AuthRefreshToken(string refreshtoken)
		{
			UriBuilder ub = new UriBuilder(Common.AUTHTOKENOP);
			Utilities.AddParam(ref ub, "client_id", Contans.DoubanApiKey);
			Utilities.AddParam(ref ub, "client_secret", Contans.DoubanSecret);
			...
			Utilities.AddParam(ref ub, "refresh_token", refreshtoken);
			string result = Utilities.RequestPost(ub.ToString());
			AuthAccessToken token = (AuthAccessToken)Utilities.JsonDeserialize<AuthAccessToken>(result);
			Common.Token = token.Token;
			Common.RefreshToken = token.RefreshToken;
